    ...
    public class ClsCommonDAL
    {
        ...
        public static ClsReturnStatusBO FunLoginValidate(ClsLoginBO ObjPriLoginBO)
        {
            ...
            try
            {
                ...
                byte[] bytesToDecode = Convert.FromBase64String(ObjPriLoginBO.Password);

                string decodedString = System.Text.Encoding.UTF8.GetString(bytesToDecode);
                /**************************************************************/
                ObjPriLoginBO.Password = decodedString;
                ...
                if (ObjDSOutPut.Tables.Count > 1)
                {
                    ...
                    if (DtUserDtl.Rows[0]["Code"].ToString() == "0")
                    {
                        ...
                        ObjPriLoginBO.EnableOtp = Convert.ToInt32(DtUserDtl.Rows[0]["EnableOtp"]);
                    }
                    else
                    {
                        ObjReturnStatus.Code = 1;
        ...
        public static string FunOtpValidation(ClsOtpvalidBO clsOtpvalid)
        {
            SqlConnection ObjConn = null;
            string s = "";
            try
            {
                FunGetConnection(ref ObjConn, 1);
                using (SqlStoredProcedure sspObj = new SqlStoredProcedure("dbo.sp_otp_validation", ObjConn, CommandType.StoredProcedure))
                {

                    sspObj.AddParameterWithValue("@p_user_name", SqlDbType.VarChar, 60, ParameterDirection.Input, clsOtpvalid.Username);
                    sspObj.AddParameterWithValue("@p_otp", SqlDbType.Int, 0, ParameterDirection.Input, (clsOtpvalid.Otp ^ 123456));
                    sspObj.AddParameterWithValue("@p_created_on", SqlDbType.DateTime, 0, ParameterDirection.Input, clsOtpvalid.Createdon);
                    sspObj.AddParameterWithValue("@p_valid_till", SqlDbType.DateTime, 0, ParameterDirection.Input, clsOtpvalid.Validtill);
                    sspObj.AddParameterWithValue("@p_otp_limit", SqlDbType.Int, 0, ParameterDirection.Input, clsOtpvalid.Limits);
                    sspObj.AddParameterWithValue("@p_otp_validate", SqlDbType.VarChar, 1, ParameterDirection.Input, clsOtpvalid.request);
                    sspObj.AddParameter("@p_otp_statusOUT", SqlDbType.VarChar, 3, ParameterDirection.Output);
                    sspObj.ExecuteNonQuery();
                    s = sspObj.Parameters["@p_otp_statusOUT"].Value.ToString();
                    sspObj.Dispose();
                }
            }
