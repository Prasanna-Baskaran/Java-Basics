...
namespace AGS.SwitchOperations
{
    public partial class AccountLinkDelinkReqAll : System.Web.UI.Page
    {
        ...
        public void FunSetResult(String mode, string Linkingflag)
        {
            try
            {
                if (!string.IsNullOrEmpty(Convert.ToString(Session["IssuerNo"])))
                {
                    ...
                    //}
                    _dtRequest.Rows.Add(new Object[] { txtAccountNo.Value, lblCIF.Text , lblCardNo.Text, lblNIC.Text, "1", ddlAccountType.SelectedValue, hdnCheckAccQuntifr.Value });
        ...
        protected void BtnSyncData_Click(object sender, EventArgs e)
        {
            try
            {
                lblCardNo.Text = lblCIF.Text = lblNIC.Text = lblNameOnCard.Text = string.Empty;

                if (string.IsNullOrWhiteSpace(txtSearchCardNo.Value) && string.IsNullOrWhiteSpace(txtSearchNameOnCard.Value) && string.IsNullOrWhiteSpace(txtCIF.Value) && string.IsNullOrWhiteSpace(txtNIC.Value))
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "javascript", "Hidemodel(); FunAddEditModalHide();", true);
                }
                else
                {
                    DataTable dtDetails = ClsAccountLinkingDelinkingBAL.FunSyncCardDetailsForAccountLinking(txtSearchCardNo.Value, txtCIF.Value, txtNIC.Value, txtSearchNameOnCard.Value, Session["IssuerNo"].ToString());

                    if (dtDetails != null && dtDetails.Rows.Count > 0)
                    {
                        lblCardNo.Text = Convert.ToString(dtDetails.Rows[0]["CardNo"]);
                        lblCIF.Text = Convert.ToString(dtDetails.Rows[0]["Cif Id"]);
                        lblNIC.Text = Convert.ToString(dtDetails.Rows[0]["NIC"]);
                        lblNameOnCard.Text = Convert.ToString(dtDetails.Rows[0]["Customer Preferred name"]);

                        ScriptManager.RegisterStartupScript(this, this.GetType(), "javascript", "Hidemodel();FunAddEditModalShow();", true);
                    }
                    else
                    {
                        LblResult.InnerHtml = "No Details Found !";
                    }
                }
            ...
            {
                new ClsCommonBAL().FunInsertIntoErrorLog("AGS.SwitchOperations.AccountLinkingRequest,BtnSyncData_Click()", exc.Message, Convert.ToString(Session["IssuerNo"]));
                LblResult.InnerHtml = "Error Occured";
            }
        }
    }
